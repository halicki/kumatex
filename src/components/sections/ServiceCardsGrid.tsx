...
export function ServiceCardsGrid() {
  return (
    <section className="mx-auto max-w-[1440px] px-16 py-16">
        ...
        {services.map((s, i) => (
          <Link key={s.slug} href={`/serwis/${s.slug}`} className="group flex flex-col items-start pb-4">
            <div className={`flex items-center p-2.5 ${i === 0 ? "bg-primary" : ""}`}>
              <Image src={iconMap[s.icon] || iconMap.excavator} alt="" width={64} height={64} unoptimized />
            </div>
            <h3 className={`mt-4 min-h-[80px] font-[family-name:var(--font-inter)] text-xl font-medium ${i === 0 ? "text-white" : "text-dark"}`}>
              {s.title}
            </h3>
